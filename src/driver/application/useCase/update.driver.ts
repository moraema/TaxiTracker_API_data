...
export class UpdateDriver {
    ...
    async run(driver_id: string, name: string, last_name: string, pin: number, image: string) {

        try {
            const updateDriver = await this.driversRepository.updateByIdDriver(driver_id, name, last_name, pin, image);
